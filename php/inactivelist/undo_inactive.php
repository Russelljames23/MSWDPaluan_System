<?php
// undo_inactive.php - Fixed version with activity logging and context detection
...
// =========================================================
// USE THE SAME CONTEXT DETECTION AS undo_deceased.php
// =========================================================
function detectUserContext($data)
{
    ...
    error_log("=== START detectUserContext (undo_inactive.php) ===");
...
try {
    ...
    $infoStmt = $conn->prepare("
        SELECT applicant_id, first_name, last_name, control_number, status, 
               date_of_inactive, inactive_reason
        FROM applicants 
        WHERE applicant_id = ?
    ");
    $infoStmt->execute([$id]);
    ...
    // Check if applicant is deceased
    if ($applicantInfo['status'] === 'Deceased') {
        throw new Exception("Cannot restore deceased applicant to active status.");
    }

    // Update status to Active and clear inactive fields
    $stmt = $conn->prepare("
        UPDATE applicants 
        SET status = 'Active', 
            date_of_inactive = NULL, 
    ...
    if ($context === 'staff') {
        $activityType = 'STAFF_UNDO_INACTIVE';
        $description = "Staff restored senior from inactive to active status";
        ...
        $activityType = 'UNDO_INACTIVE';
        $description = "Admin restored senior from inactive to active status";
    ...
    $logDetails = [
        ...
        'previous_inactive_date' => $applicantInfo['date_of_inactive'] ?? null,
        'previous_inactive_reason' => $applicantInfo['inactive_reason'] ?? null,
    ...
    if ($logger) {
        ...
        $logger->log($errorActivityType, 'Failed to restore senior from inactive status', [
            'applicant_id' => $id,
    ...
    error_log("❌ Database error in undo_inactive.php: " . $e->getMessage());
    ...
    error_log("❌ Error in undo_inactive.php: " . $e->getMessage());
