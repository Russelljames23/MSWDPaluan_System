<?php
// ActivityLogger.php - Fixed version
class ActivityLogger
{
    ...
    public function log($activityType, $description, $details = null)
    {
        try {
            ...
            // Clean up any null bytes in user agent
            ...
            error_log("ActivityLogger: Log inserted - Type: {$activityType}, User: {$userId}");
            return $stmt->rowCount() > 0;
        } catch (Exception $e) {
            error_log("ActivityLogger Error: " . $e->getMessage());
            // Log the SQL error details
            if (isset($stmt)) {
                error_log("ActivityLogger SQL Error Info: " . json_encode($stmt->errorInfo() ?? []));
            }
            return false;
        }
    }
