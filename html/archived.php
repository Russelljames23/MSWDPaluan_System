...
<body class="bg-gray-50 dark:bg-gray-900">
            ...
            <ul class="pt-5 mt-5 space-y-2 border-t border-gray-200 dark:border-gray-700">
                <li>
                    <a href="#"
                        ...
                        <i class="fas fa-archive w-6 h-6  text-blue-700 dark:text-white group-hover:text-blue-800 dark:group-hover:text-white"></i>
            ...
            <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
                <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
                    ...
                    <div
                        ...
                        <h4 class="text-xl font-medium text-gray-900 dark:text-white">Archived List</h4>
                        ...
                        <div class="relative w-full md:w-auto">
                            ...
                            <div id="archivedFilterDropdownMenu"
                                class="hidden absolute z-10 mt-2 w-44 bg-white rounded-lg shadow dark:bg-gray-800 dark:border dark:border-gray-700">
                    ...
                    <div class="overflow-x-auto">
                        <table id="archivedTable" class="w-full text-sm text-left text-gray-700 dark:text-gray-400">
                            ...
                            <tbody id="archivedBody">
                                <tr class="border-b text-xs font-medium text-center border-gray-200 dark:border-gray-700">
                                    <!-- Dynamic Rows -->
                                </tr>
                            </tbody>
                        </table>
                    ...
                    <nav id="paginationNav"
                        class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-3 border-t border-gray-200 dark:border-gray-700"
    ...
    <div id="confirmModal"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md p-6 text-center transform scale-95 opacity-0 transition-all duration-300 ease-out border border-gray-200 dark:border-gray-700"
            id="confirmBox">
            <h2 id="confirmTitle" class="text-xl font-semibold mb-3 text-gray-800 dark:text-white">Please Confirm</h2>
            <p id="confirmMessage" class="text-gray-600 dark:text-gray-300 mb-6 leading-relaxed"></p>
            <div class="flex justify-center gap-3">
                <button id="confirmCancelBtn"
                    class="px-4 py-1 bg-blue-600 cursor-pointer text-white text-xs rounded-sm font-medium hover:bg-blue-700 transition-all focus:outline-none focus:ring-2 focus:ring-blue-300 dark:bg-blue-700 dark:hover:bg-blue-800">
                    Cancel
                </button>
                <button id="confirmOkBtn"
                    class="px-4 py-1 bg-green-600 cursor-pointer text-white text-xs rounded-sm font-medium hover:bg-green-700 transition-all focus:outline-none focus:ring-2 focus:ring-green-400 dark:bg-green-700 dark:hover:bg-green-800">
                    Confirm
                </button>
            </div>
        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            async function fetchArchived() {
                try {
                    ...
                    if (!data.archived || data.archived.length === 0) {
                        tbody.innerHTML = `<tr><td colspan="9" class="text-center py-4 text-gray-500 dark:text-gray-400">No archived records found.</td></tr>`;
                    ...
                    paginatedData.forEach((row, index) => {
                        ...
                        tr.className = "border-b text-xs font-medium text-center border-gray-200 dark:border-gray-700";
                        tr.setAttribute("data-id", row.applicant_id);
                        tr.innerHTML = `
                        <td class="px-2 py-1 text-gray-900 dark:text-gray-300">${start + index + 1}</td>
                        <td class="px-2 py-1 text-gray-900 dark:text-gray-300">${row.full_name}</td>
                        <td class="px-2 py-1 text-gray-900 dark:text-gray-300">${row.birth_date || ""}</td>
                        <td class="px-2 py-1 text-gray-900 dark:text-gray-300">${row.age || ""}</td>
                        <td class="px-2 py-1 text-gray-900 dark:text-gray-300">${row.gender || ""}</td>
                        <td class="px-2 py-1 text-gray-900 dark:text-gray-300">${row.civil_status || ""}</td>
                        <td class="px-2 py-1 text-gray-900 dark:text-gray-300">${row.archived_date || ""}</td>
                        <td class="px-2 py-1 text-red-600 dark:text-red-400">${row.validation}</td>
                        <td class="px-2 py-1 text-center">
                            <button onclick="restoreArchived(${row.applicant_id}, '${row.full_name}')"
                                class="px-3 py-1 cursor-pointer bg-green-600 text-white text-xs rounded hover:bg-green-700 transition dark:bg-green-700 dark:hover:bg-green-800">
                                ♻️ Restore
                            </button>
                        </td>`;
                    ...
                    console.error(err);
                    tbody.innerHTML = `<tr><td colspan="9" class="text-center text-red-500 dark:text-red-400 py-4">⚠️ Error loading data.</td></tr>`;
                    paginationNav.innerHTML = "";
                }
            }
            ...
            function renderPagination(start, end) {
                ...
                <li class="relative group">
                    ...
                    <span class="absolute bottom-full mb-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150 
                        text-xs text-black dark:text-white dark:bg-gray-700 bg-white border border-gray-200 dark:border-gray-600 px-2 py-1 rounded shadow-lg">
                        Previous page
                    </span>
                </li>`;
                ...
                for (let i = 1; i <= totalPages; i++) {
                    ...
                    <button data-page="${i}" class="flex items-center justify-center text-sm py-2 px-3 leading-tight 
                        ${i === currentPage
                        ? 'z-10 text-blue-600 bg-blue-50 border border-blue-300 hover:bg-blue-100 hover:text-blue-700 dark:bg-blue-900 dark:text-blue-100 dark:border-blue-700 dark:hover:bg-blue-800'
            ...
            window.restoreArchived = async function(id, name) {
                showConfirm("Restore Record", `Do you want to restore ${name} to the Recent List?`, async () => {
                    try {
            ...
            // ---------- FILTER DROPDOWN ----------
            const filterButton = document.getElementById("archivedFilterDropdownButton");
            const filterMenu = document.getElementById("archivedFilterDropdownMenu");
